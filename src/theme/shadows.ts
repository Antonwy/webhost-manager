...
const createCustomShadow = (color: string): CustomShadow => {
  ...
  const { primary, info, secondary, success, warning, error, common } =
    palette.light;
  ...
  return {
    ...
    success: `0 8px 16px 0 ${alpha(
      (success as SimplePaletteColorOptions).main,
      0.24
    )}`,
    warning: `0 8px 16px 0 ${alpha(
      (warning as SimplePaletteColorOptions).main,
      0.24
    )}`,
    error: `0 8px 16px 0 ${alpha(
      (error as SimplePaletteColorOptions).main,
      0.24
    )}`,
    //
